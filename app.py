...
# Configuración de la página con el color de tu marca en la pestaña
st.set_page_config(page_title="Calculadora Art Center", layout="wide", page_icon="✂️")

# --- PERSONALIZACIÓN VISUAL (Tus colores corporativos) ---
# #fed80c (Amarillo Oficenter), #e9769d (Rosa), #74b7d5 (Azul), #8bcc60 (Verde)
st.markdown(f"""
    <style>
        ...
        /* Personalizar las pestañas (Tabs) */
        button[data-baseweb="tab"] {{
            color: #74b7d5 !important;
            font-size: 16px !important;
            font-weight: bold !important;
        }}
        button[data-baseweb="tab"][aria-selected="true"] {{
            color: #e9769d !important;
            border-bottom-color: #e9769d !important;
        }}
...
# Colocamos un encabezado visual con la esencia de tu logo
st.markdown("<h1 style='text-align: center; color: #e9769d; font-size: 42px; margin-bottom: 0px;'>🎨 ART CENTER</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #74b7d5; font-weight: bold; font-size: 18px; margin-top: 0px;'>Costos & Presupuestos de Papelería Creativa</p>", unsafe_allow_html=True)
st.divider()
...
with tab1:
    ...
    with col2:
        ...
        nombre_producto = st.text_input("Nombre del producto final", placeholder="Ej. Topper 2 capas 15cm")
...
# ==========================================
# TAB 2: INVENTARIO DE MATERIALES
# ==========================================
with tab2:
    ...
    with col_inv:
        ...
        for nombre, datos in st.session_state.materiales.items():
            ...
            tabla_visual.append({
                "Material": nombre, "Tipo": datos["Tipo"],
                "Costo Base": f"${datos['Costo Proveedor']:.2f}", "Precio Tienda": f"${datos['Precio Tienda']:.2f}",
                "Medidas": f"{datos['Ancho (cm)']}x{datos['Alto (cm)']} cm" if datos["Tipo"] == "Por Área (cm²)" else "N/A",
                "Costo x " + unidad_medida: f"${costo_uni:.5f}", "Precio x " + unidad_medida: f"${precio_uni:.5f}"
            })
        st.dataframe(pd.DataFrame(tabla_visual), use_container_width=True)
        
    with col_form:
        st.subheader("⚙️ Configurar Insumo")
        mat_nombre = st.text_input("Nombre del Material")
        ...
        if mat_tipo == "Por Área (cm²)":
            ...
            mat_ancho = c_an.number_input("Ancho (cm)", min_value=1.0, value=50.0)
            mat_alto = c_al.number_input("Alto (cm)", min_value=1.0, value=70.0)
        ...
        if st.button("💾 Guardar Material"):
            if mat_nombre:
                ...
                st.success(f"¡'{mat_nombre}' guardado!")
                st.rerun()
...
# ==========================================
with tab3:
    st.header("📜 Catálogo de Diseños Guardados")
